...
using CommonCore.CrossCuttingConcerns.Validation;
using CommonCore.Utilities.Interceptors;
using CommonCore.Utilities.Messages;
using CommonCore.Utilities.Results;
using CommonCore.Core.Entities;

namespace CommonCore.Aspects.Autofac.Validation
{
    public class ValidationAspect : MethodInterception
    {
        ...
        protected override void OnException(IInvocation invocation, System.Exception e)
        {
            if (e is ValidationException validationException)
            {
                ...
                {
                    invocation.ReturnValue = new ErrorDataResult<BaseEntity>(null, true, validationException.Errors.Select(s => s.ErrorMessage).ToList());  
                }
            }
            else
            {
